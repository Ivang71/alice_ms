...
export async function searchAlice(browser: any, locale: string, acceptLanguage: string, query: string, timeoutMs: number, signal: AbortSignal | undefined, getAiAnswer: boolean): Promise<string> {
  ...
  try {
    ...
    return await searchAliceWithContext(context, page, locale, acceptLanguage, query, timeoutMs, signal, getAiAnswer)
  } finally {
    try { await context.close() } catch {}
  }
}

export async function searchAliceWithContext(context: any, page: any, locale: string, acceptLanguage: string, query: string, timeoutMs: number, signal: AbortSignal | undefined, getAiAnswer: boolean): Promise<string> {
